...
//     if(!m_isMedium) {
//       throw NoriException("Volpath integrator expect a medium that wrap the scene\n");
//     }
//   }
...
//       float tmax = its.t;
//       float t = -log(1 - sample->next1D()) / med->m_sigmaT[0]; //TODO: sigmaT by channel
...
//           float transmittance = exp(-med->m_sigmaT[0] * (y - its.p).norm()); //TODO: Take random channel instead
...
//         _ray.o = its.p;
//         // _ray.d = wo; //TODO: Investigate why this is not working
//       } else if(its.mesh->isMedium()) {
  ...
  Color3f Li(const Scene *scene, Sampler *sample, const Ray3f &ray) const {
    ...
    for(int depth = 0;; depth++) {
      ...
      if(t < tmax) {
        ...
        _ray.o = its.p;
        _ray.d = wo; //TODO: Investig/ate why this is not working for envmap
